...
function validateSetting(
  ...
  if (typeof value !== setting.type) {
    return {
      valid: false,
      message: `Invalid type for ${key}: expected ${setting.type}`,
    };
  }
  ...
  if (setting.allowedValues && !setting.allowedValues.includes(value)) {
    return {
      valid: false,
      message: `Invalid value for ${key}: allowed values are ${setting.allowedValues.join(", ")}`,
    };
  }
...
function getSettings(req: Request, res: Response) {
  ...
  try {
    ...
    return res
      .status(200)
      .json(OK("Settings successfully retrieved", { settings }));
  } catch (error) {
    return res
      .status(500)
      .json(InternalServerError("Failed to retrieve settings"));
  }
}
...
function updateSettings(req: Request, res: Response) {
  ...
  try {
    ...
    return res
      .status(200)
      .json(
        OK("Settings updated successfully", { settings: { lastModified } }),
      );
  } catch (error) {
    return res
      .status(500)
      .json(InternalServerError("Failed to update settings"));
  }
}
...
function getProfile(req: Request, res: Response) {
  ...
  try {
    ...
    return res
      .status(200)
      .json(OK("Profile retrieved successfully", { profile: user }));
  } catch (error) {
    return res
      .status(500)
      .json(InternalServerError("Failed to retrieve profile"));
  }
}
