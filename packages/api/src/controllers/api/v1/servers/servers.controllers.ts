...
function getServers(req: Request, res: Response) {
  ...
  return res
    .status(200)
    .json(OK("Successfully retrieved servers", { servers }));
}
...
function openServer(req: Request, res: Response) {
  ...
  if (!roblox_server_id || !roblox_place_id || !name) {
    return res
      .status(400)
      .json(BadRequest("Missing roblox_server_id, roblox_place_id, or name"));
  }
  ...
  const server = serversService.getServerByRobloxServerIdAndPlaceId(
    roblox_server_id,
    place.place_id,
  );
