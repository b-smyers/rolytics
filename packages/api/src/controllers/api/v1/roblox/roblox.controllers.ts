...
async function getPlaceDetails(req: Request, res: Response) {
  ...
  try {
    ...
    const imageIds: (string | number)[] = media.data.map(
      (item: any) => item.imageId,
    );

    // Step 4: Get the actual image URLs from the CDN
    let imageUrls: string[];
    try {
      imageUrls = await robloxService.getMediaThumbnails(
        experienceId,
        imageIds,
      );
    } catch (error) {
    ...
    return res.status(200).json(
      OK("Place details successfully retrieved", {
        placeId: placeId,
        name: details.name,
