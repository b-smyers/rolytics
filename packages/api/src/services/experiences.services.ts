...
function createExperience(
  ...
  logger.info(
    `Experience '${name}' created for user ID '${user_id}' with Roblox Experience ID '${roblox_experience_id}'`,
  );
...
function deleteExperience(experience_id: number): boolean {
  ...
  if (changes > 0) {
    ...
    logger.warn(
      `Attempted to delete non-existent experience with ID '${experience_id}'`,
    );
  }
...
function updateExperience(
  ...
  logger.info(
    `Experience with ID '${experience_id}' updated with fields: [${fields.map((f) => f.split(" ")[0]).join(", ")}]`,
  );
}
...
function getExperienceById(experience_id: number): Experience | undefined {
  ...
  const experience = db.prepare(query).get(experience_id) as
    | DBExperience
    | undefined;
...
function getExperiencesByUserId(
  user_id: number,
  limit = 10,
): Experience[] | undefined {
  const query = `SELECT * FROM experiences WHERE user_id = ? LIMIT ?`;
  const experiences = db.prepare(query).all(user_id, limit) as
    | DBExperience[]
    | undefined;

  if (experiences) {
    logger.info(
      `Fetched ${experiences.length} experience(s) for user ID '${user_id}' with limit ${limit}`,
    );
  } else {
    logger.info(
      `No experience(s) for user ID '${user_id}' with limit ${limit}`,
    );
  }
