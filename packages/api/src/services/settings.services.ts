...
function getSettings(userId: number): Settings {
  ...
  (result as { setting_key: string; setting_value: string }[]).forEach(
    ({ setting_key, setting_value }) => {
      try {
        ...
        logger.warn(
          `Failed to parse setting value for key '${setting_key}' for user ${userId}`,
        );
      }
    },
  );
...
function updateSettings(userId: number, settings: Settings): number {
  ...
  const transaction = db.transaction((settings: Settings) => {
    Object.entries(settings).forEach(([key, value]) => {
      ...
      if (result.changes === 0) {
        logger.warn(
          `Setting update skipped because setting '${key}' does not exist for user ${userId}`,
        );
      }
    });
  });
