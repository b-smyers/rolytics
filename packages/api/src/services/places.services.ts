...
function createPlace(
  ...
  logger.info(
    `Place '${name}' created with Roblox Place ID ${roblox_place_id} and Experience ID ${experience_id}`,
  );
...
function getPlacesByExperienceId(
  experience_id: number,
  limit: number = 10,
): Place[] | undefined {
  ...
  const places: DBPlace[] = db
    .prepare(query)
    .all(experience_id, limit) as DBPlace[];

  if (places) {
    logger.info(
      `Fetched ${places.length} place(s) for Experience ID ${experience_id} with limit ${limit}`,
    );
  } else {
    logger.info(
      `No place(s) for experience ID '${experience_id}' with limit ${limit}`,
    );
  }
