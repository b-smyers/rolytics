...
function createServer(
  ...
  logger.info(
    `Server '${name}' created with Roblox Server ID ${roblox_server_id} and Place ID ${place_id}`,
  );
...
function updateServer(
  ...
  logger.info(
    `Server with ID ${server_id} updated with fields: [${updates.map((u) => u.split(" ")[0]).join(", ")}]`,
  );
}
...
function getServerByRobloxServerIdAndPlaceId(
  roblox_server_id: string,
  place_id: number,
): Server | undefined {
  ...
  const server: Server = db
    .prepare(query)
    .get(roblox_server_id, place_id) as Server;

  if (server) {
    logger.info(
      `Fetched server with Roblox Server ID ${roblox_server_id} and Place ID ${place_id}`,
    );
  } else {
    logger.warn(
      `No server found with Roblox Server ID ${roblox_server_id} and Place ID ${place_id}`,
    );
  }
...
function getServersByPlaceId(place_id: number, limit = 10): Server[] {
  ...
  logger.info(
    `Fetched ${servers.length} server(s) for Place ID ${place_id} with limit ${limit}`,
  );
