...
async function getMediaThumbnails(
  experienceId: string | number,
  imageIds: (string | number)[],
): Promise<string[]> {
  if (imageIds.length === 0) {
    logger.warn(
      `getMediaThumbnails: Empty imageIds array provided for experienceId ${experienceId}`,
    );
  ...
  try {
    ...
    logger.info(
      `Fetched media thumbnails for experienceId ${experienceId} with ${imageIds.length} thumbnailIds`,
    );
    return (response.data?.data || []).map(
      (thumbnail: any) => thumbnail.imageUrl,
    );
  } catch (error: any) {
    logger.error(
      `getMediaThumbnails: Error fetching thumbnails for experienceId ${experienceId} - ${error.message}`,
    );
    return Promise.resolve([]);
  }
}
...
async function getExperienceMedia(experienceId: string | number): Promise<any> {
  ...
  try {
    ...
    logger.error(
      `getExperienceMedia: Error fetching media for experienceId ${experienceId} - ${error.message}`,
    );
    throw error;
  }
}
...
async function getExperienceDetails(
  ...
  try {
    ...
    logger.error(
      `getExperienceDetails: Error fetching details for experienceId ${experienceId} - ${error.message}`,
    );
    throw error;
  }
}

async function getExperienceIdfromPlaceId(
  ...
  try {
    ...
    logger.error(
      `getExperienceIdfromPlaceId: Error fetching universeId for placeId ${placeId} - ${error.message}`,
    );
    throw error;
  }
}

async function getPlacesByRobloxExperienceId(
  experienceId: string | number,
): Promise<any[]> {
  ...
  try {
    const response = await client.get(url);
    logger.info(
      `Fetched places for experienceId ${experienceId} with ${response.data.data.length} place(s)`,
    );
    return response.data.data;
  } catch (error: any) {
    logger.error(
      `getPlacesByRobloxExperienceId: Error fetching places for experienceId ${experienceId} - ${error.message}`,
    );
    throw error;
  }
}
