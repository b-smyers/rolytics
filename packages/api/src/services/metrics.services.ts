...
import { DBMetadataMetric, DBMetric, DBPerformanceMetric, DBPlayersMetric, DBPurchasesMetric, DBSocialMetric, Metadata, MetadataMetric, Metric, Performance, PerformanceMetric, Players, PlayersMetric, Purchases, PurchasesMetric, Social, SocialMetric } from 'types/metrics';
...
function getMetricsByServerId(server_id: number, limit = 20): Metric[] | undefined {
    ...
    const metrics = db.prepare(query).all(server_id, limit) as DBMetric[] | undefined;
...
function getPerformanceMetricsByServerId(server_id: number, limit = 20): PerformanceMetric[] | undefined {
    const query = `SELECT timestamp, server_id, performance FROM metrics WHERE server_id = ? ORDER BY timestamp DESC LIMIT ?`;
    const metrics = db.prepare(query).all(server_id, limit) as DBPerformanceMetric[] | undefined;
...
function getPurchasesMetricsByServerId(server_id: number, limit = 20): PurchasesMetric[] | undefined {
    const query = `SELECT timestamp, server_id, purchases FROM metrics WHERE server_id = ? ORDER BY timestamp DESC LIMIT ?`;
    const metrics = db.prepare(query).all(server_id, limit) as DBPurchasesMetric[] | undefined;
...
function getSocialMetricsByServerId(server_id: number, limit = 20): SocialMetric[] | undefined {
    const query = `SELECT timestamp, server_id, social FROM metrics WHERE server_id = ? ORDER BY timestamp DESC LIMIT ?`;
    const metrics = db.prepare(query).all(server_id, limit) as DBSocialMetric[] | undefined;
...
function getPlayersMetricsByServerId(server_id: number, limit = 20): PlayersMetric[] | undefined {
    const query = `SELECT timestamp, server_id, players FROM metrics WHERE server_id = ? ORDER BY timestamp DESC LIMIT ?`;
    const metrics = db.prepare(query).all(server_id, limit) as DBPlayersMetric[] | undefined;
...
function getMetadataMetricsByServerId(server_id: number, limit = 20): MetadataMetric[] | undefined {
    const query = `SELECT timestamp, server_id, metadata FROM metrics WHERE server_id = ? ORDER BY timestamp DESC LIMIT ?`;
    const metrics = db.prepare(query).all(server_id, limit) as DBMetadataMetric[] | undefined;
